        ...
        with Center():
            with Middle():
                with Vertical(id="serverSelectBox"):
                    ...
                    yield Input(placeholder="Enter 1, 2 or 3...", id="serverSelectInput")
                    yield Label("", id="serverSelectMessage", classes="popupHelp")

    def on_mount(self) -> None:
        self.query_one("#serverSelectInput", Input).focus()

    def setMessage(self, text: str) -> None:
        self.query_one("#serverSelectMessage", Label).update(text)
...
class CustomServerScreen(ModalScreen[str | None]):
    def compose(self) -> ComposeResult:
        with Center():
            with Middle():
                with Vertical(id="customServerBox"):
                    yield Label("Enter custom server URL", classes="popupTitle")
